...
const actionBrandController = {
    ...
    createBrand: async (req, res) => {
        try {
            ...
            const [rows] = await pool.query('INSERT INTO brand (brand) VALUES (?)', [brand]);
            res.send({ rows });
        } catch (error) {
    ...
    updateBrand: async (req, res) => {
        ...
        try {
            const { id } = req.params;
            const { brand } = req.body;
            const [result] = await pool.query('UPDATE brand SET brand = IFNULL(?,brand) WHERE id = ?', [brand, id]);
            //console.log(result)
...
export default actionBrandController